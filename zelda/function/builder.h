...
#define ZELDA_DETAIL_FUNCTION_CLASS_POLY_EACH(r, data, i, x) ZELDA_DETAIL_FUNCTION_CLASS_INVOKE((i, ZELDA_PP_REM data, x))
// TODO: Msvc invoke
...
ZELDA_DETAIL_FUNCTION_CLASS(ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAME(i, name), n, params, body)

#define ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAME(i, name) BOOST_PP_CAT(zelda_poly_, BOOST_PP_CAT(i, name))
#define ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAMES(n, name) BOOST_PP_ENUM(n, ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAMES_EACH, name)
#define ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAMES_EACH(z, i, name) ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAME(i, name)

#define ZELDA_DETAIL_FUNCTION_CLASS_POLY(name, params, bodies) \
ZELDA_PP_DEFER(ZELDA_PP_SEQ_FOR_EACH_ID)()(ZELDA_DETAIL_FUNCTION_CLASS_POLY_EACH, \
    (ZELDA_PP_REM name, ZELDA_PP_NARGS params, ZELDA_PP_ARGS_TO_SEQ params), \
    bodies) \
    typedef zelda::poly_adaptor<ZELDA_DETAIL_FUNCTION_CLASS_POLY_NAMES(BOOST_PP_SEQ_SIZE(bodies), ZELDA_PP_REM name) > ZELDA_PP_REM name; 

#define ZELDA_DETAIL_FUNCTION_CLASS_POLY_P(s, data, x) ZELDA_PP_EQUAL(x, else)
...
ZELDA_DETAIL_FUNCTION_CLASS_POLY(name, BOOST_PP_SEQ_ELEM(0, seq), ZELDA_PP_SEQ_SPLIT(BOOST_PP_SEQ_TAIL(seq), ZELDA_DETAIL_FUNCTION_CLASS_ALL_P, data))
